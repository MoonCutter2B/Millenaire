...
public class DevModUtilities {
	...
	private static class DevPathedEntity implements IAStarPathedEntity {
		...
		public void onFoundPath(final List<AStarNode> result) {
			...
			for (final AStarNode node : result) {
				if (node != result.get(0)
						&& node != result.get(result.size() - 1)) {
					MillCommonUtilities.setBlockAndMetadata(world, new Point(
							node).getBelow(), Blocks.wool, meta);
				}
			}
		}
		...
		public void onNoPathAvailable() {
			ServerSender.sendChat(caller, EnumChatFormatting.DARK_RED,
					"No path available.");
		}
	...
	public static void fillInFreeGoods(final EntityPlayer player) {
		...
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseWarriorBlueLegs, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseWarriorBlueBoots, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseWarriorBlueHelmet, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseWarriorBluePlate, 1);

		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseWarriorRedLegs, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseWarriorRedBoots, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseWarriorRedHelmet, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseWarriorRedPlate, 1);

		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseGuardLegs, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseGuardBoots, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseGuardHelmet, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.japaneseGuardPlate, 1);

		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.summoningWand, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.skoll_hati_amulet, 1);
		MillCommonUtilities.putItemsInChest(player.inventory, Items.clock, 1);
		MillCommonUtilities
				.putItemsInChest(player.inventory, Mill.normanAxe, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.normanPickaxe, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Mill.normanShovel, 1);
		MillCommonUtilities.putItemsInChest(player.inventory,
				Blocks.gold_block, 0, 64);
		...
		MillCommonUtilities.putItemsInChest(player.inventory,
				Blocks.cobblestone, 128);
		MillCommonUtilities
				.putItemsInChest(player.inventory, Blocks.stone, 512);
		...
		MillCommonUtilities
				.putItemsInChest(player.inventory, Mill.maize, 0, 64);
		MillCommonUtilities.putItemsInChest(player.inventory, Mill.turmeric, 0,
				64);
				...
				if (autoMoveDirection.containsKey(p)) {
					...
					if (autoMoveDirection.get(p) == 1) {
						if (autoMoveTarget.get(p) < p.posX) {
							...
							ServerSender.sendChat(p, EnumChatFormatting.GREEN,
									"Auto-move: turning back.");
						}
						...
						if (autoMoveTarget.get(p) > p.posX) {
							...
							autoMoveTarget.put(p, (int) (p.posX + 100000));
							ServerSender.sendChat(p, EnumChatFormatting.GREEN,
									"Auto-move: turning back again.");
						}
					}

					((EntityPlayerMP) p).playerNetServerHandler
							.setPlayerLocation(
									p.posX + autoMoveDirection.get(p) * 0.5,
									p.posY, p.posZ, p.rotationYaw,
									p.rotationPitch);
				}
			}
		}
	...
	public static void testPaths(final EntityPlayer player) {
			...
			for (int j = 0; j < 100 && (start == null || end == null); j++) {
				for (int k = 0; k < 100 && (start == null || end == null); k++) {
					for (int l = 0; l < 8 && (start == null || end == null); l++) {
						final Point p = centre.getRelative(i
								* (1 - (l & 1) * 2), j * (1 - (l & 2)), k
								* (1 - (l & 4) / 2));

						final Block block = MillCommonUtilities.getBlock(
								player.worldObj, p);
		...
		if (start != null && end != null) {
			...
			final DevPathedEntity pathedEntity = new DevPathedEntity(
			...
			ServerSender.sendChat(player, EnumChatFormatting.DARK_GREEN,
					"Calculating path. Tolerance H: "
							+ jpsConfig.toleranceHorizontal);

			final AStarPathPlanner jpsPathPlanner = new AStarPathPlanner(
					player.worldObj, pathedEntity);
			jpsPathPlanner.getPath(start.getiX(), start.getiY(), start.getiZ(),
					end.getiX(), end.getiY(), end.getiZ(), jpsConfig);
		} else {
			ServerSender.sendChat(player, EnumChatFormatting.DARK_RED,
					"Could not find start or end: " + start + " - " + end);
		}
	...
	public static void villagerInteractDev(final EntityPlayer entityplayer,
			final MillVillager villager) {
		if (villager.isChild()) {
			villager.growSize();
			ServerSender.sendChat(entityplayer, EnumChatFormatting.GREEN,
					villager.getName() + ": Size: " + villager.size
		...
		if (entityplayer.inventory.getCurrentItem() == null) {
			ServerSender.sendChat(
					...
					villager.getName() + ": Current goal: "
							+ villager.getGoalLabel(villager.goalKey)
							+ " Current pos: " + villager.getPos());
			ServerSender.sendChat(entityplayer, EnumChatFormatting.GREEN,
					villager.getName() + ": House: " + villager.housePoint
							+ " Town Hall: " + villager.townHallPoint);
			ServerSender.sendChat(entityplayer, EnumChatFormatting.GREEN,
					villager.getName() + ": ID: " + villager.villager_id);
			if (villager.getRecord() != null) {
				ServerSender.sendChat(
						entityplayer,
						EnumChatFormatting.GREEN,
						villager.getName() + ": Spouse: "
								+ villager.getRecord().spousesName);
			}

			if (villager.getPathDestPoint() != null
					&& villager.pathEntity != null
					&& villager.pathEntity.getCurrentPathLength() > 1) {
				ServerSender.sendChat(
						entityplayer,
						EnumChatFormatting.GREEN,
						villager.getName()
								...
								+ " distance: "
								+ villager.getPathDestPoint().distanceTo(
										villager) + " stuck: "
								+ villager.longDistanceStuck + " jump:"
								+ villager.pathEntity.getNextTargetPathPoint());
			} else {
				ServerSender.sendChat(entityplayer, EnumChatFormatting.GREEN,
						villager.getName() + ": No dest point.");
			}
			...
			if (villager.mw.getProfile(entityplayer.getDisplayName()).villagersInQuests
					.containsKey(villager.villager_id)) {
				s += " quest: "
						+ villager.mw.getProfile(entityplayer.getDisplayName()).villagersInQuests
								.get(villager.villager_id).quest.key
						+ "/"
						+ villager.mw.getProfile(entityplayer.getDisplayName()).villagersInQuests
								.get(villager.villager_id).getCurrentVillager().id;
			}
			...
			for (final PathPoint p : villager.pathEntity.pointsCopy) {
				if (villager.worldObj
						.getBlock(p.xCoord, p.yCoord - 1, p.zCoord) != Mill.lockedChest) {
					MillCommonUtilities.setBlockAndMetadata(villager.worldObj,
							new Point(p).getBelow(), Blocks.wool, meta);
				}
			}
			PathPoint p = villager.pathEntity.getCurrentTargetPathPoint();
			if (p != null
					&& villager.worldObj.getBlock(p.xCoord, p.yCoord - 1,
							p.zCoord) != Mill.lockedChest) {
				MillCommonUtilities.setBlockAndMetadata(villager.worldObj,
						new Point(p).getBelow(), Blocks.gold_block, 0);
			}
			...
			if (p != null
					&& villager.worldObj.getBlock(p.xCoord, p.yCoord - 1,
							p.zCoord) != Mill.lockedChest) {
				MillCommonUtilities.setBlockAndMetadata(villager.worldObj,
						new Point(p).getBelow(), Blocks.diamond_block, 0);
			}
			...
			if (p != null
					&& villager.worldObj.getBlock(p.xCoord, p.yCoord - 1,
							p.zCoord) != Mill.lockedChest) {
				MillCommonUtilities.setBlockAndMetadata(villager.worldObj,
						new Point(p).getBelow(), Blocks.iron_block, 0);
			}
		}
		...
		if (villager.hasChildren()
				...
				&& entityplayer.inventory.getCurrentItem().getItem() == Mill.summoningWand) {
			final MillVillager child = villager.getHouse().createChild(
					villager, villager.getTownHall(),
					villager.getRecord().spousesName);
			if (child != null) {
				child.size = 20;
				child.growSize();
