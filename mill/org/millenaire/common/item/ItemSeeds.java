...
public class ItemSeeds extends ItemText
{
	...
	public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int i, int j, int k, int l, float hitX, float hitY, float hitZ)
	{
		...
		if (!profile.isTagSet(MillWorld.CROP_PLANTING+cropKey)) {
			if (!world.isRemote) {
		...
		{
			world.setBlockAndMetadataWithNotify(i, j + 1, k, cropID, cropMeta, 2);
			itemstack.stackSize--;
