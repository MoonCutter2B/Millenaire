...
public class GoalSleep extends Goal {
	...
	public GoalInformation getDestination(MillVillager villager) throws Exception {
		...
		final Point sleepPos=villager.getHouse().getSleepingPos();

		final Vector<Point> beds=new Vector<Point>();
		...
		if (beds.size()>0)
			return packDest(beds.firstElement(),villager.getHouse());

		final Vector<Point> feetPos=new Vector<Point>();
				...
				for (int k=0;k<6;k++) {
					for (int l=0;l<8;l++) {
						...
						if (!p.isBlockPassable(world)
								&& p.getAbove().isBlockPassable(world) && p.getRelative(0, 2, 0).isBlockPassable(world)) {

							Point topBlock=MillCommonUtilities.findTopNonPassableBlock(world, p.getiX(), p.getiZ());

							//must be a roof above (to avoid villagers sleeping outdoor)
							if ((topBlock!=null) && (topBlock.y>(p.y+1))) {
								...
								if (!p2.isBlockPassable(world)
										&& p2.getAbove().isBlockPassable(world) && p2.getRelative(0, 2, 0).isBlockPassable(world)) {
		...
		if (feetPos.size()>0)
			return packDest(feetPos.firstElement(),villager.getHouse());
