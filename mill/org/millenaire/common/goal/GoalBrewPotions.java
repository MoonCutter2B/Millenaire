...
public class GoalBrewPotions extends Goal {
	...
	public GoalInformation getDestination(MillVillager villager) throws Exception {
		...
		final int nbBottles=villager.getHouse().countGoods(Items.glass_bottle);
		final int nbPotions=villager.getHouse().countGoods(Items.potionitem,-1);
		...
		for (final Point p : villager.getHouse().brewingStands) {
			...
			if ((brewingStand !=null) && (brewingStand.getBrewTime()==0)) {
				...
				for (int i=0;i<3;i++) {
					if ((brewingStand.getStackInSlot(i)!=null) && (brewingStand.getStackInSlot(i).getItem()==Items.potionitem) &&
			...
			if ((nbBottles>2) && (nbPotions<64)) {
				for (int i=0;i<3;i++) {
					if (brewingStand.getStackInSlot(i)==null) {
						brewingStand.setInventorySlotContents(i,new ItemStack(Items.potionitem,1));
