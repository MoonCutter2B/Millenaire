...
public class PathingSurface {
	...
	//Subclass of Chunk, we use this to save a lot of memory. In this case only the tiles of the surface are saved,
	//all other tiles(which were null in the array) are not saved. Furthermore are all tiles lost which are not reachable from the administration building.
	...
	//used to save all tiles that are left in our linked data structure, we need this to effectively find tiles
	//if we calculate ways from tiles which are not the central tile
	public LinkedList<ExtendedPathTile> alltiles;

	//input: three dimensional array[x][y][z](x=width, y=height, z=depth) of tiles and the tile which is the central tile to calculate and only save possible ways
	//IMPORTANT: An entry of the array has to be null if the tile is passable (air,grass,flowers...)!
	//
	public PathingSurface(PathingPathCalcTile[][][] region, PathingPathCalcTile ct)
	{
		...
		//A tile get's part of the surface if a player is able to be standing on it(every solid tile above
		//which two tiles are empty(null)), those tiles are not necessarily to be reached from every other tile!
		...
		for(int i=0;i<region.length;i++)
		{
			for(int j=0;j<(region[0].length-2);j++)
			{
				...
				{
					//given condition is supposed to be true if the checked tile is not null, both spaces above the tile are empty or the tile is a ladder
					if((region[i][j][k]!=null)
							&&//CDJ note: added () arround the || condition to ensure that region[i][j][k] is non-null. Or should chck be addd before third part?
							//HD note: you are right, there was a () missing.
		...
		while(!toprocess.isEmpty())
		{
			...
			for(byte t=-1;t<=1;t++)
			{
				//a tile below a ladder always gets added if it's not empty, a tile above only if it is itself a ladder and has no solid tile above
				...
				else
				{
					...
					if(((j+t)>=0)&&((j+t)<surface[0].length))
					{
						if((i+1)<surface.length)
						{
							...
							{
								//if the tile is a ladder, we only see it as neighbor if the tile is one field higher(because we don't walk on the surface of a ladder)
								//we've already checked that above any viewed ladder field is at least 1 field empty
								...
								else
								{
									...
									//checking when adding neighbors if 3 tiles are empty if we change height
									if(t==0)
									{
										current.neighbors.add(surface[i+1][j+t][k]);
									}
	...
	//Input: the coordinates of start and target position as an array (x,y,z) of shorts.
	//Note: the target location is always the field on which the player stands at the end of his journey.
	//for example: we are going outside to get wood, the start location is the tile on which we currently stand,
	//or if we are currently using a ladder, the field we are currently in. The target location is always a field
	...
	public LinkedList<short[]> getPath(short[] start, short[] target)
	{
		...
		//implementing a modified binary search to find the starting tile if it is in our surface
		...
		//Linked List to save the Chunks whose neighbors we have not yet evaluated
		...
		//variable to remind which chunk was on the way before the current tiles
