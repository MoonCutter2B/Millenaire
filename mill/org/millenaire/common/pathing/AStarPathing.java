...
public class AStarPathing {
	...
	static public class CachedPath {
		...
		CachedPath(Vector<Point2D> v) {
			...
			final Vector<Point2D> v2=new Vector<Point2D>();
	...
	public class PathingWorker extends Thread {
		...
		public Vector<PathPoint> getPathViaNodes(MillVillager villager, int pStartX,int pStartZ,int pDestX,int pDestZ) throws Exception {
			...
			if (end.neighbours.size()==0)
			{
				...
				final Vector<Node> testedNodes=new Vector<Node>();
			...
			final Vector<Node> open=new Vector<Node>();
			...
			while (open.size()>0) {
				...
				for (final Node n : cn.neighbours) {
					...
					if (closed.containsKey(n)) {
						//Log.debug(Log.getPath, n+" has already been visited.");
					} else {
						if (!open.contains(n)) {
	...
	private static Vector<Point2D> buildPointsNode(Node end) {
		...
		if (n.from!=null) {
			...
			final Vector<Point2D> path=new Vector<Point2D>();
	...
	private Vector<PathPoint> buildFinalPath(MillVillager villager,CachedPath path)  throws Exception {
		...
		final Vector<PathPoint> ppoints=new Vector<PathPoint>();
		...
		for (final Node n : nodes) {
			...
			if ((n.cornerSide==1) && (n.pos.x < (winfo.length-1)) && (n.pos.z < (winfo.width-1))) {
				if (bottom[n.pos.x][n.pos.z] && right[n.pos.x][n.pos.z]
						&& bottom[n.pos.x][n.pos.z+1]  && right[n.pos.x+1][n.pos.z]) {//next diagonal available
			...
			if ((n.cornerSide==4) && (n.pos.x < (winfo.length-1)) && (n.pos.z > 0)) {
				if (bottom[n.pos.x][n.pos.z] && left[n.pos.x][n.pos.z]
						&& bottom[n.pos.x][n.pos.z-1]  && left[n.pos.x+1][n.pos.z]) {//next diagonal available
					final int tx=n.pos.x+1;
					final int tz=n.pos.z-1;
			...
			if ((n.cornerSide==8) && (n.pos.x > 0) && (n.pos.z > 0)) {
				if (top[n.pos.x][n.pos.z] && left[n.pos.x][n.pos.z]
						&& top[n.pos.x][n.pos.z-1]  && left[n.pos.x-1][n.pos.z]) {//next diagonal available
					final int tx=n.pos.x-1;
					final int tz=n.pos.z-1;
	...
	public boolean createConnectionsTable(MillWorldInfo winfo, Point thStanding) throws MillenaireException {
		...
		/* Deep copy of the topGround array from world info because it is updated more often than pathing
		 * And since the pathing system uses it to find the height of the final path, if it has been updated in the meantime
		 * it can produce weird result (such as a path point at the top of a tree that has grown in the meantime)
		 */

		topGround=MillWorldInfo.shortArrayDeepClone(winfo.topGround);

		nodes=new Vector<Node>();
		firstDemand=new Vector<PathKey>();
		firstDemandOutcome=new Vector<String>();
	...
	private void displayConnectionsLog() {
		...
		//						Log.minor(this,Log.Connections, "Not symmetrical for top/bottom at "+i+"/"+j);
		//				}
		//				if (j>0) {
		//					if (left[i][j] != right[i][j-1])
		//						Log.minor(this,Log.Connections, "Not symmetrical for left/right at "+i+"/"+j);
		//				}
		//			}
		//		}
	}
	...
	private void findRegions(Point thStanding) throws MillenaireException {
		...
		while (nodesMarked < nodes.size()) {
			...
			final Vector<Node> toVisit=new Vector<Node>();
	...
	public boolean isInArea(Point p) {
		return !( (p.x < winfo.mapStartX) || (p.x >= (winfo.mapStartX+winfo.length))
				|| (p.z < winfo.mapStartZ) || (p.z >= (winfo.mapStartZ+winfo.width)));
	}
	...
	private void storeInCache(CachedPath path,Point2D dest) throws Exception {
		...
		//MLLog.major(null, MLLog.Pathing, "Storing main path: "+path+" key: "+path.getKey());
		...
		for (int i=1;i<(path.points.length-2);i++) {
			...
			if (!dest.equals(cp.getEnd()))
			{
				...
				//MLLog.major(null, MLLog.Pathing, "Storing secondary path: "+cp+" key: "+cp.getKey());
			}
		}
	}
