...
/**
 * Extension of the Minecraft PathEntity to try and fix some of its horrible
 * shortcomings
 * 
 * 
...
public class AS_PathEntity extends PathEntity {
	...
	public AS_PathEntity(final PathPoint[] points) {
		super(points);
	...
	public PathPoint getFuturePathPoint(final int jump) {
		if (getCurrentPathIndex() >= pointsCopy.length - jump) {
			return null;
		}
	...
	public PathPoint getNextTargetPathPoint() {
		if (getCurrentPathIndex() >= pointsCopy.length - 1) {
			return null;
		}
		return this.pointsCopy[getCurrentPathIndex() + 1];
	}

	public PathPoint getPastTargetPathPoint(final int jump) {
		if (getCurrentPathIndex() < jump || pointsCopy.length == 0) {
			return null;
		}
		return this.pointsCopy[getCurrentPathIndex() - jump];
	}
	...
	@Override
	public Vec3 getPosition(final Entity var1) {
		if (super.isFinished()) {
			return null;
		}
	...
	public PathPoint getPreviousTargetPathPoint() {
		if (getCurrentPathIndex() < 1 || pointsCopy.length == 0) {
			return null;
		}
		return this.pointsCopy[getCurrentPathIndex() - 1];
	}
