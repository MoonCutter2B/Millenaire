	...
	public static void main(final String[] args) {
		...
		try {
			...
			while ((s = reader.readLine().trim()) != null) {
				if (!s.startsWith("//")) {
					if (s.split(";").length > 1) {
						final short[] start = new short[] { (short) Integer.parseInt(s.split(";")[0].split("/")[0]), (short) Integer.parseInt(s.split(";")[0].split("/")[1]),
								(short) Integer.parseInt(s.split(";")[0].split("/")[2]) };
						final short[] end = new short[] { (short) Integer.parseInt(s.split(";")[1].split("/")[0]), (short) Integer.parseInt(s.split(";")[1].split("/")[1]),
								(short) Integer.parseInt(s.split(";")[1].split("/")[2]) };
