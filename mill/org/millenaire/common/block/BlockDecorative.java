...
public class BlockDecorative extends Block {
	...
	public void alchemistExplosion(final World world, final int i, final int j,
			final int k) {

		MillCommonUtilities.setBlockAndMetadata(world, i, j, k, Blocks.air, 0,
				true, false);
			...
			if (y + j >= 0 && y + j < 128) {
				for (int x = -EXPLOSION_RADIUS; x <= EXPLOSION_RADIUS; x++) {
					for (int z = -EXPLOSION_RADIUS; z <= EXPLOSION_RADIUS; z++) {
						if (x * x + y * y + z * z <= EXPLOSION_RADIUS
								* EXPLOSION_RADIUS) {
							final Block block = world.getBlock(i + x, j + y, k
									+ z);
							if (block != Blocks.air) {
								MillCommonUtilities.setBlockAndMetadata(world,
										i + x, j + y, k + z, Blocks.air, 0,
										true, false);
							}
						}
					}
	...
	@Override
	public void dropBlockAsItemWithChance(final World par1World, final int i,
			final int j, final int k, final int meta, final float par6,
			final int par7) {
	...
	public void updateTick(final World world, final int i, final int j,
		...
		if (this == Mill.earth_decoration && meta == 0) {
			if (world.getBlockLightValue(i, j + 1, k) >= 15) {
				if (MillCommonUtilities.chanceOn(5)) {
					MillCommonUtilities.setBlockAndMetadata(world, i, j, k,
							Mill.stone_decoration, 1, true, false);
				}
			}
			...
			if (world.getBlockLightValue(i, j + 1, k) < 7) {
				if (MillCommonUtilities.chanceOn(5)) {
					MillCommonUtilities.setBlockAndMetadata(world, i, j, k,
							Mill.wood_decoration, 4, true, false);
				}
			}
		}
