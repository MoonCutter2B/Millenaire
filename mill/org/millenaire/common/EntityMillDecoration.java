...
public class EntityMillDecoration extends EntityPainting implements
		IEntityAdditionalSpawnData {

	public static enum EnumWallDecoration {
		Griffon("Griffon", 16, 16, 0, 0, NORMAN_TAPESTRY), Oiseau("Oiseau", 16,
				16, 16, 0, NORMAN_TAPESTRY), CorbeauRenard("CorbeauRenard",
				2 * 16, 16, 2 * 16, 0, NORMAN_TAPESTRY), Serment("Serment",
				5 * 16, 3 * 16, 0, 16, NORMAN_TAPESTRY), MortHarold(
				"MortHarold", 4 * 16, 3 * 16, 5 * 16, 16, NORMAN_TAPESTRY), Drakar(
				"Drakar", 6 * 16, 3 * 16, 9 * 16, 16, NORMAN_TAPESTRY), MontStMichel(
				"MontStMichel", 3 * 16, 2 * 16, 0, 4 * 16, NORMAN_TAPESTRY), Bucherons(
				"Bucherons", 3 * 16, 2 * 16, 3 * 16, 4 * 16, NORMAN_TAPESTRY), Cuisine(
				"Cuisine", 3 * 16, 2 * 16, 6 * 16, 4 * 16, NORMAN_TAPESTRY), Flotte(
				"Flotte", 15 * 16, 3 * 16, 0, 6 * 16, NORMAN_TAPESTRY), Chasse(
				"Chasse", 6 * 16, 3 * 16, 0, 9 * 16, NORMAN_TAPESTRY), Siege(
				"Siege", 16 * 16, 3 * 16, 0, 12 * 16, NORMAN_TAPESTRY),

		Ganesh("Ganesh", 2 * 16, 3 * 16, 0, 0, INDIAN_STATUE), Kali("Kali",
				2 * 16, 3 * 16, 2 * 16, 0, INDIAN_STATUE), Shiva("Shiva",
				2 * 16, 3 * 16, 4 * 16, 0, INDIAN_STATUE), Osiyan("Osiyan",
				2 * 16, 3 * 16, 6 * 16, 0, INDIAN_STATUE), Durga("Durga",
				2 * 16, 3 * 16, 8 * 16, 0, INDIAN_STATUE),

		MayanTeal("MayanTeal", 2 * 16, 2 * 16, 0, 3 * 16, MAYAN_STATUE), MayanGold(
				"MayanGold", 2 * 16, 2 * 16, 2 * 16, 3 * 16, MAYAN_STATUE),

		LargeJesus("LargeJesus", 2 * 16, 3 * 16, 0, 5 * 16,
				BYZANTINE_ICON_LARGE), LargeVirgin("LargeVirgin", 2 * 16,
				3 * 16, 2 * 16, 5 * 16, BYZANTINE_ICON_LARGE), MediumVirgin1(
				"MediumVirgin1", 2 * 16, 2 * 16, 0, 8 * 16,
				BYZANTINE_ICON_MEDIUM), MediumVirgin2("MediumVirgin2", 2 * 16,
				2 * 16, 2 * 16, 8 * 16, BYZANTINE_ICON_MEDIUM), SmallJesus(
				"SmallJesus", 16, 16, 0, 10 * 16, BYZANTINE_ICON_SMALL), SmallVirgin1(
				"SmallVirgin1", 16, 16, 16, 10 * 16, BYZANTINE_ICON_SMALL), SmallVirgin2(
				"SmallVirgin2", 16, 16, 2 * 16, 10 * 16, BYZANTINE_ICON_SMALL), SmallVirgin3(
		...
		private EnumWallDecoration(final String s1, final int j, final int k,
				final int l, final int i1, final int type) {
			title = s1;
			sizeX = j;
			sizeY = k;
			offsetX = l;
	...
	public static EntityMillDecoration createTapestry(final World world,
			Point p, final int type) {
		...
		return new EntityMillDecoration(world, p.getiX(), p.getiY(), p.getiZ(),
				orientation, type, true);
	}
	...
	public static int guessOrientation(final World world, final Point p) {
		...
		if (MillCommonUtilities.isBlockIdSolid(world.getBlock(i - 1, j, k))) {
			return 3;
		} else if (MillCommonUtilities.isBlockIdSolid(world.getBlock(i + 1, j,
				k))) {
			return 1;
		} else if (MillCommonUtilities.isBlockIdSolid(world.getBlock(i, j,
				k - 1))) {
			return 2;
		} else if (MillCommonUtilities.isBlockIdSolid(world.getBlock(i, j,
				k + 1))) {
	...
	public EntityMillDecoration(final World par1World, final int par2,
	...
	public EntityMillDecoration(final World world, final int x, final int y,
			final int z, final int orientation, final int type,
	...
	public EntityMillDecoration(final World world, final int i, final int j,
			final int k, final int l, final int type, final String s) {
		super(world, i, j, k, l);
		this.type = type;
	...
	public void dropItemStack() {
		if (type == NORMAN_TAPESTRY) {
			worldObj.spawnEntityInWorld(new EntityItem(worldObj, posX, posY,
					posZ, new ItemStack(Mill.tapestry)));
		} else if (type == INDIAN_STATUE) {
			worldObj.spawnEntityInWorld(new EntityItem(worldObj, posX, posY,
					posZ, new ItemStack(Mill.indianstatue)));
		} else if (type == MAYAN_STATUE) {
			worldObj.spawnEntityInWorld(new EntityItem(worldObj, posX, posY,
					posZ, new ItemStack(Mill.mayanstatue)));
		} else if (type == BYZANTINE_ICON_SMALL) {
			worldObj.spawnEntityInWorld(new EntityItem(worldObj, posX, posY,
					posZ, new ItemStack(Mill.byzantineiconsmall)));
		} else if (type == BYZANTINE_ICON_MEDIUM) {
			worldObj.spawnEntityInWorld(new EntityItem(worldObj, posX, posY,
					posZ, new ItemStack(Mill.byzantineiconmedium)));
		} else if (type == BYZANTINE_ICON_LARGE) {
			worldObj.spawnEntityInWorld(new EntityItem(worldObj, posX, posY,
					posZ, new ItemStack(Mill.byzantineiconlarge)));
		}
	}
