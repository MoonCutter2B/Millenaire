...
public class EntityMillVillager extends EntityCreature {
	...
	/*
	 * @Override public void attackEntity(final Entity entity, final float f) { if
	 * (vtype.isArcher && f > 5 && hasBow()) { attackEntityBow(entity, f);
	 * isUsingBow = true; } else { if (attackTime <= 0 && f < 2.0F &&
	 * entity.boundingBox.maxY > boundingBox.minY && entity.boundingBox.minY <
	 * boundingBox.maxY) { attackTime = 20;
	 * entity.attackEntityFrom(DamageSource.causeMobDamage(this),
	 * getAttackStrength()); swingItem(); } isUsingHandToHand = true; } }
	 */

	/*
	 * public void attackEntityBow(final Entity entity, final float f) { if
	 * (!(entity instanceof EntityLivingBase)) { return; }
	 * 
	 * if (f < 10F) { final double d = entity.posX - posX; final double d1 =
	 * entity.posZ - posZ; if (attackTime == 0) {
	 * 
	 * float speedFactor = 1; float damageBonus = 0;
	 * 
	 * final ItemStack weapon = getWeapon();
	 * 
	 * if (weapon != null) { final Item item = weapon.getItem();
	 * 
	 * if (item instanceof ItemMillenaireBow) { final ItemMillenaireBow bow =
	 * (ItemMillenaireBow) item;
	 * 
	 * if (bow.speedFactor > speedFactor) { speedFactor = bow.speedFactor; } if
	 * (bow.damageBonus > damageBonus) { damageBonus = bow.damageBonus; } } }
	 * 
	 * final EntityArrow arrow = new EntityArrow(this.worldObj, this,
	 * (EntityLivingBase) entity, 1.6F, 12.0F);
	 * 
	 * this.worldObj.playSoundAtEntity(this, "random.bow", 1.0F, 1.0F /
	 * (this.getRNG().nextFloat() * 0.4F + 0.8F));
	 * this.worldObj.spawnEntityInWorld(arrow);
	 * 
	 * attackTime = 60;
	 * 
	 * // faster MLN arrows arrow.motionX *= speedFactor; arrow.motionY *=
	 * speedFactor; arrow.motionZ *= speedFactor;
	 * 
	 * // extra arrow damage arrow.setDamage(arrow.getDamage() + damageBonus); }
	 * rotationYaw = (float) (Math.atan2(d1, d) * 180D / 3.1415927410125732D) - 90F;
	 * hasAttacked = true; } }
	 */

	/*
	 * @Override public boolean attackEntityFrom(final DamageSource ds, final float
	 * i) {
	 * 
	 * if (ds.getSourceOfDamage() == null && ds != DamageSource.outOfWorld) { return
	 * false; }
	 * 
	 ...
	 * 
	 * if (entity != null) { if (entity instanceof EntityLivingBase) { if (entity
	 * instanceof EntityPlayer) { lastAttackByPlayer = true;
	 * 
	 * final EntityPlayer player = (EntityPlayer) entity;
	 * 
	 * if (!isRaider) { if (!vtype.hostile) {
	 * MillCommonUtilities.getServerProfile(player.worldObj,
	 * player.getDisplayName()).adjustReputation(getTownHall(), (int) (-i * 10)); }
	 * if (worldObj.difficultySetting != EnumDifficulty.PEACEFUL && this.getHealth()
	 * < getMaxHealth() - 10) { entityToAttack = entity; clearGoal(); if
	 * (getTownHall() != null) { getTownHall().callForHelp(entity); } }
	 * 
	 * if (hadFullHealth && (player.getHeldItem() == null ||
	 * MillCommonUtilities.getItemWeaponDamage(player.getHeldItem().getItem()) <= 1)
	 * && !worldObj.isRemote) { ServerSender.sendTranslatedSentence(player,
	 * MLN.ORANGE, "ui.communicationexplanations"); } }
	 * 
	 * if (lastAttackByPlayer && getHealth() <= 0) { if (vtype.hostile) {
	 * player.addStat(MillAchievements.selfdefense, 1); } else {
	 * player.addStat(MillAchievements.darkside, 1); } }
	 * 
	 * } else { entityToAttack = entity; clearGoal();
	 * 
	 * if (getTownHall() != null) { getTownHall().callForHelp(entity); }
	 * 
	 * } } }
	 * 
	 * return b; }
	...
	/*
	 * public void detrampleCrops() { if (getPosition().sameBlock(prevPoint) &&
	 * (previousBlock == Blocks.wheat || previousBlock instanceof BlockMillCrops) &&
	 * getBlock(getPosition()) != Blocks.air && getBlock(getPosition().getBelow())
	 * == Blocks.dirt) { setBlock(getPosition(), previousBlock);
	 * setBlockMetadata(getPosition(), previousBlockMeta);
	 * setBlock(getPosition().getBelow(), Blocks.farmland); }
	 * 
	 * previousBlock = getBlock(getPosition()); previousBlockMeta =
	 * getBlockMeta(getPosition()); }
	 */

	// emptied to prevent generic code from turning the villagers' heads toward
	// the player
	// @Override
	// public void faceEntity(final Entity par1Entity, final float par2, final float
	// par3) {}
	...
	// Foreign Merchant leaves at night if stock is empty && price goes up by 1.5 if
	// in a different culture

	// Find function for checking armor equipment, should return the equipment if in
	// inventory

	// GetBedOrientation()?

	// Set up where Villagers have an appropriate tool in inventory, tool updates as
	// village expands/building upgrades
	...
	/*
	 * public Item[] getGoodsToBringBackHome() { return vtype.bringBackHomeGoods; }
	 * 
	 * public Item[] getGoodsToCollect() { return vtype.collectGoods; }
	 */
	 ...
	 * 
	 * //if (getTownHall().controlledBy(player.getDisplayName())) { // cost = cost /
	 * 2; //}
	 * 
	 * return cost; }
	 */
	...
	/*
	 * public String getName() { return firstName + " " + familyName; }
	 */

	// handleDoorsAndFenceGates() needs to be malisis-compatible(dummy player with
	// villagers rotationYaw)
	...
	public boolean interact(final EntityPlayer playerIn) {
		...
		if (type.hireCost > 0) {
			...
			playerIn.openGui(Millenaire.instance, 5, playerIn.worldObj, this.getPosition().getX(),
					this.getPosition().getY(), this.getPosition().getZ());
			return true;
		}
		if (type.isChief) {
			this.isPlayerInteracting = true;
			playerIn.openGui(Millenaire.instance, 4, playerIn.worldObj, this.getPosition().getX(),
					this.getPosition().getY(), this.getPosition().getZ());
			return true;
		}
		...
		// Display Trade Window if Trading (Foreign Merchant, trading for Townhall or
		// local shop)
		return false;
	}
	...
	// When Villager dies, the entity is dead, per normal. Drop stuff and display
	// messages. Respawn must just create another instance of the same villager
	// (reason to store culture info in V. Stone)
	// Why villagerID is important

	// Local merchants have inn or townhall as 'house', handle moving them, taking
	// items from townhall, and what happens if inn is full
	...
	public void onLivingUpdate() {
		...
		// setFacingDirection(); Look toward goal (or entity to attack, but I think
		// attacking something is a goal)(entityToAttack also used in raids and against
		// stupid players, may still be possible to use goal)
	...
	// teenager leaving to find other village...possibly useful in creating new
	// villages?

	// be smart with teleportTo, use coordinates or entity, check for surrounding
	// blocks
	...
	public void onUpdate() {
		...
		if (isPlayerInteracting) {
			List<EntityPlayer> playersNear = this.worldObj.getEntitiesWithinAABB(EntityPlayer.class,
					new AxisAlignedBB(posX - 5, posY - 1, posZ - 5, posX + 5, posY + 1, posZ + 5));
		...
		/*
		 * if (hiredBy != null) { //updateHired(); super.onUpdate(); return; }
		 */
		...
		// Chance to speak something SPEAKING SENTENCES AND DIALOGUE CAN BE
		// CLIENT-SIDE!!
		// pick up items?
		// updateLocalMerchant()
		...
		// trigger Mob Attacks on Villagers (currently only spiders, which is
		// odd...villagers will seek out other mobs, but should be targetable).
	...
	// performNightActions() does not appear to be called in the code
	// anymore...perhaps it has been outdated? This undoes growChildSize,
	// conception, and ForiegnMerchantNightAction
	...
	// Update Texture for Byzantines with silk clothes, possibly further expand on
	// this
	...
	public static void preinitialize() {
		EntityRegistry.registerModEntity(EntityMillVillager.class, "millVillager", 0, Millenaire.instance, 80, 3,
				false);
	}
