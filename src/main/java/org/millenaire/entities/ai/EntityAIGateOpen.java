...
public class EntityAIGateOpen extends EntityAIBase {
	...
	/**
	 * If is true then the Entity has stopped Gate Interaction and completed the
	 * task.
	 */
	...
	/**
	 * The temporisation before the entity close the door (in ticks, always 20 = 1
	 * second)
	 */
		...
		if (!this.theEntity.isCollidedHorizontally) {
			...
			if (pathentity != null && !pathentity.isFinished() && pathnavigateground.getEnterDoors()) {
				...
				for (int i = 0; i < Math.min(pathentity.getCurrentPathIndex() + 2,
					...
					if (this.theEntity.getDistanceSq((double) this.gatePosition.getX(), this.theEntity.posY,
							(double) this.gatePosition.getZ()) <= 2.25D) {
