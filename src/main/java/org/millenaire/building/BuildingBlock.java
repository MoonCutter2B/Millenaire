...
public class BuildingBlock {
	...
	public void build(World worldIn, boolean onGeneration) {
		if (specialBlock != BuildingBlock.PRESERVEGROUNDDEPTH && specialBlock != BuildingBlock.PRESERVEGROUNDSURFACE
			...
			worldIn.playSoundEffect(position.getX() + 0.5D, position.getY() + 0.5D, position.getZ() + 0.5D, soundName,
					0.3F, 0.6F);
		}
		...
		if (specialBlock == BuildingBlock.PRESERVEGROUNDDEPTH || specialBlock == BuildingBlock.PRESERVEGROUNDSURFACE) {
			...
			if (validGroundBlock == null) {
				...
				worldIn.playSoundEffect(position.getX() + 0.5D, position.getY() + 0.5D, position.getZ() + 0.5D,
						soundName, 0.3F, 0.6F);
			} else if (onGeneration && validGroundBlock == Blocks.dirt
					&& worldIn.getBlockState(position.up()) == null) {
			...
			if (onGeneration) {
				...
				WorldGenerator wg = new WorldGenTrees(true, 4 + CommonUtilities.random.nextInt(7),
			...
			if (onGeneration) {
				WorldGenerator wg = new WorldGenTrees(false, 4 + CommonUtilities.random.nextInt(7),
