...
public class MillWalkNodeProcessor extends WalkNodeProcessor {
	...
	public int findPathOptions(PathPoint[] pathOptions, Entity entityIn, PathPoint currentPoint, PathPoint targetPoint,
		...
		if (this.getMillVerticalOffset(entityIn, currentPoint.xCoord, currentPoint.yCoord + 1,
				currentPoint.zCoord) == 1) {
			j = 1;
		}

		PathPoint pathpoint = this.getMillSafePoint(entityIn, currentPoint.xCoord, currentPoint.yCoord,
				currentPoint.zCoord + 1, j);
		PathPoint pathpoint1 = this.getMillSafePoint(entityIn, currentPoint.xCoord - 1, currentPoint.yCoord,
				currentPoint.zCoord, j);
		PathPoint pathpoint2 = this.getMillSafePoint(entityIn, currentPoint.xCoord + 1, currentPoint.yCoord,
				currentPoint.zCoord, j);
		PathPoint pathpoint3 = this.getMillSafePoint(entityIn, currentPoint.xCoord, currentPoint.yCoord,
				currentPoint.zCoord - 1, j);
	...
	private PathPoint getMillSafePoint(Entity entityIn, int x, int y, int z, int p_176171_5_) {
		...
		if (i == 2) {
			...
			if (pathpoint == null && p_176171_5_ > 0 && i != -3 && i != -4
					&& this.getMillVerticalOffset(entityIn, x, y + p_176171_5_, z) == 1) {
	...
	private int getMillVerticalOffset(Entity entityIn, int x, int y, int z) {
		...
		if (block instanceof BlockFenceGate && this.blockaccess.isAirBlock(pos.up()))
			...
			return func_176170_a(this.blockaccess, entityIn, x, y, z, this.entitySizeX, this.entitySizeY,
					this.entitySizeZ, this.getAvoidsWater(), true, this.getEnterDoors());
	}
}
