...
public class VillageGeography {
	...
	private void createWorldInfo(final List<BuildingLocation> locations, final BuildingLocation blIP, final int pstartX,
			final int pstartZ, final int endX, final int endZ) {
	...
	static public boolean isForbiddenBlockForConstruction(final Block block) {
		return block == Blocks.water || block == Blocks.flowing_water || block == Blocks.ice
				...
				|| block == BlockDecorative.blockDecorativeEarth || block == BlockDecorative.blockDecorativeStone
				|| block == BlockDecorative.blockDecorativeWood || block == BlockDecorative.byzantineTile
				|| block == BlockDecorative.byzantineTileSlab || block == BlockDecorative.byzantineStoneTile
				|| block == BlockDecorative.paperWall || block == BlockDecorative.emptySericulture;
	}
	...
	public boolean update(final World world, final List<BuildingLocation> locations, final BuildingLocation blIP,
			final BlockPos center, final int radius) {
	...
	private void updateChunk(final int startX, final int startZ) {
		...
		final Chunk chunk = world
				.getChunkFromBlockCoords(new BlockPos(startX + mapStartX, yBaseline, startZ + mapStartZ));
		...
		for (int i = 0; i < 16; i++) {
			for (int j = 0; j < 16; j++) {
				...
				while (y <= maxy && y > 1
						&& !(!isBlockSolid(chunk.getBlock(i, y, j)) && !isBlockSolid(chunk.getBlock(i, y + 1, j)))) {
					y++;
				}
				...
				if (soilBlock == BlockMillPath.blockMillPath || soilBlock == BlockMillPath.blockMillPathSlab
						|| soilBlock == BlockMillPath.blockMillPathSlabDouble) {
					path[mx][mz] = true;
				} else {
					path[mx][mz] = false;
				}
				...
				if (!(soilBlock instanceof BlockFence) && !(soilBlock instanceof BlockWall) && !isBlockSolid(block)
						&& block != Blocks.flowing_water && soilBlock != Blocks.water) {
					spaceAbove[mx][mz] = 1;
				} else {
					blocked = true;
				}
				...
				if (!danger[mx][mz] && !buildingLoc[mx][mz]) {
					if (constructionHeight[mx][mz] > yBaseline - VALIDHEIGHTDIFF
							&& constructionHeight[mx][mz] < yBaseline + VALIDHEIGHTDIFF) {
						canBuild[mx][mz] = true;
					}
				}
		...
		while (gapFilled) {
			...
			for (int i = -5; i < 21; i++) {
				for (int j = -5; j < 21; j++) {
					...
					if (mz >= 0 && mz < width) {
						if (mx > 1 && mx < length - 1) {
							if (Math.abs(topGround[mx - 1][mz] - topGround[mx + 1][mz]) < 2
									&& (topGround[mx - 1][mz] + 2 < topGround[mx][mz]
											|| topGround[mx + 1][mz] + 2 < topGround[mx][mz])) {
								final short ntg = topGround[mx - 1][mz];
								final boolean samesolid = isBlockSolid(world
										.getBlockState(
												new BlockPos(startX + mapStartX + i, ntg, startZ + mapStartZ + j))
										.getBlock());
								final boolean belowsolid = isBlockSolid(world
										.getBlockState(
												new BlockPos(startX + mapStartX + i, ntg - 1, startZ + mapStartZ + j))
										.getBlock());
								final boolean below2solid = isBlockSolid(world
										.getBlockState(
												new BlockPos(startX + mapStartX + i, ntg - 2, startZ + mapStartZ + j))
										.getBlock());
								final boolean abovesolid = isBlockSolid(world
										.getBlockState(
												new BlockPos(startX + mapStartX + i, ntg + 1, startZ + mapStartZ + j))
										.getBlock());
								final boolean above2solid = isBlockSolid(world
										.getBlockState(
												new BlockPos(startX + mapStartX + i, ntg + 2, startZ + mapStartZ + j))
										.getBlock());
								final boolean above3solid = isBlockSolid(world
										.getBlockState(
												new BlockPos(startX + mapStartX + i, ntg + 3, startZ + mapStartZ + j))
										.getBlock());

								// check if same level works
								if (Math.abs(topGround[mx - 1][mz] - topGround[mx + 1][mz]) < 2 && belowsolid
								...
								} else if (topGround[mx + 1][mz] <= topGround[mx - 1][mz] && below2solid && !belowsolid
										&& !samesolid && !abovesolid) {
									...
									gapFilled = true;
									topAdjusted[mx][mz] = true;
								} else if (topGround[mx + 1][mz] >= topGround[mx - 1][mz] && samesolid && !abovesolid
										&& !above2solid) {
									topGround[mx][mz] = (short) (ntg + 1);
									if (!above3solid) {
										spaceAbove[mx][mz] = 3;
					...
					if (mx >= 0 && mx < length) {
						if (mz > 1 && mz < width - 1) {
							if (Math.abs(topGround[mx][mz - 1] - topGround[mx][mz + 1]) < 3
								...
								// check if same level works
								if (Math.abs(topGround[mx][mz - 1] - topGround[mx][mz + 1]) < 2 && belowsolid
										&& !samesolid && !abovesolid) {
									topGround[mx][mz] = ntg;
									if (!above2solid) {
										spaceAbove[mx][mz] = 3;
									...
									gapFilled = true;
									topAdjusted[mx][mz] = true;
								} else if (topGround[mx][mz + 1] <= topGround[mx][mz - 1] && below2solid && !belowsolid
										&& !samesolid && !abovesolid) {
									topGround[mx][mz] = (short) (ntg - 1);
									if (!abovesolid) {
										spaceAbove[mx][mz] = 3;
									...
									gapFilled = true;
									topAdjusted[mx][mz] = true;
								} else if (topGround[mx][mz + 1] >= topGround[mx][mz - 1] && samesolid && !abovesolid
										&& !above2solid) {
									topGround[mx][mz] = (short) (ntg + 1);
									if (!above3solid) {
										spaceAbove[mx][mz] = 3;
			...
			/*
			 * Then, gaps two blocks large, on the same level (for instance, passage between
			 * a double-size wall)
			 */
			for (int i = -5; i < 21; i++) {
				for (int j = -5; j < 21; j++) {
					...
					if (mz >= 0 && mz < width) {
						if (mx > 1 && mx < length - 2) {
							if (topGround[mx - 1][mz] == topGround[mx + 2][mz]
								...
								// beyond the chunk
								final boolean nextsamesolid = isBlockSolid(world
										.getBlockState(
												new BlockPos(startX + mapStartX + i + 1, ntg, startZ + mapStartZ + j))
										.getBlock());
								final boolean nextbelowsolid = isBlockSolid(world.getBlockState(
										new BlockPos(startX + mapStartX + i + 1, ntg - 1, startZ + mapStartZ + j))
										.getBlock());
								final boolean nextabovesolid = isBlockSolid(world.getBlockState(
										new BlockPos(startX + mapStartX + i + 1, ntg + 1, startZ + mapStartZ + j))
										.getBlock());
								final boolean nextabove2solid = isBlockSolid(world.getBlockState(
										new BlockPos(startX + mapStartX + i + 1, ntg + 2, startZ + mapStartZ + j))
										.getBlock());

								// check if same level works
								if (belowsolid && nextbelowsolid && !samesolid && !nextsamesolid && !abovesolid
										&& !nextabovesolid) {
									topGround[mx][mz] = ntg;
					...
					if (mx >= 0 && mx < length) {
						if (mz > 1 && mz < width - 2) {
							if (topGround[mx][mz - 1] == topGround[mx][mz + 2]
								...
								final boolean nextsamesolid = isBlockSolid(world
										.getBlockState(
												new BlockPos(startX + mapStartX + i, ntg, startZ + mapStartZ + j + 1))
										.getBlock());
								final boolean nextbelowsolid = isBlockSolid(world.getBlockState(
										new BlockPos(startX + mapStartX + i, ntg - 1, startZ + mapStartZ + j + 1))
										.getBlock());
								final boolean nextabovesolid = isBlockSolid(world.getBlockState(
										new BlockPos(startX + mapStartX + i, ntg + 1, startZ + mapStartZ + j + 1))
										.getBlock());
								final boolean nextabove2solid = isBlockSolid(world.getBlockState(
										new BlockPos(startX + mapStartX + i, ntg + 2, startZ + mapStartZ + j + 1))
										.getBlock());
	...
	public static boolean isBlockSolid(Block block) {
		return block.isFullCube() || block == Blocks.glass || block == Blocks.glass_pane || block instanceof BlockSlab
				|| block instanceof BlockStairs || block instanceof BlockFence || block instanceof BlockWall
				|| block == BlockDecorative.paperWall;
	}
