...
public class BlockOrientedSlab extends BlockSlab {
	...
	public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
		if (this.isDouble()) {
			...
			boolean flag = isSlabX(iblockstate.getBlock())
					&& iblockstate.getValue(HALF) == BlockOrientedSlab.EnumBlockHalf.TOP;
			boolean flag1 = isSlabX(iblockstate1.getBlock())
					&& iblockstate1.getValue(HALF) == BlockOrientedSlab.EnumBlockHalf.TOP;
			return flag1
					? (side == EnumFacing.DOWN ? true
							: (side == EnumFacing.UP && super.shouldSideBeRendered(worldIn, pos, side) ? true
									: !isSlabX(iblockstate.getBlock()) || !flag))
	...
	protected BlockState createBlockState() {
		return this.isDouble() ? new BlockState(this, new IProperty[] { SEAMLESS, FACING })
				: new BlockState(this, new IProperty[] { HALF, FACING });
	}
}