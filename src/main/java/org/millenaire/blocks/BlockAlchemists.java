...
public class BlockAlchemists extends Block {
	...
	public void alchemistExplosion(final World world, final int i, final int j, final int k) {
		...
		world.spawnParticle(EnumParticleTypes.EXPLOSION_HUGE, i + 0.5D, j + 0.5D, k + 0.5D, 0.0D, 0.0D, 0.0D,
				new int[0]);
		world.playSoundEffect(i, j, k, "random.explode", 8.0F,
				(1.0F + (world.rand.nextFloat() - world.rand.nextFloat()) * 0.2F) * 0.9F);
	...
	public static void preinitialize() {
		blockAlchemists = new BlockAlchemists().setCreativeTab(Millenaire.tabMillenaire)
				.setUnlocalizedName("blockAlchemists");
	...
	public static void render() {
		...
		renderItem.getItemModelMesher().register(Item.getItemFromBlock(blockAlchemists), 0,
				new ModelResourceLocation(Reference.MOD_ID + ":blockAlchemists", "inventory"));
	}
}
