...
public class BlockVillageStone extends BlockContainer {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (worldIn.isRemote)
			playerIn.addChatMessage(
					new ChatComponentText("§8The Village name almost seems to shimmer in the twilight"));
		...
		if (worldIn.getTileEntity(pos) instanceof TileEntityVillageStone) {
			...
			if (te.willExplode) {
				...
				worldIn.createExplosion(
						new EntityTNTPrimed(worldIn, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, null),
						pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, 2.0F, true);
			}
		} else {
			System.err.println("Negation failed.  TileEntity not loaded correctly.");
			return;
	...
	public static void preinitialize() {
		villageStone = new BlockVillageStone().setCreativeTab(Millenaire.tabMillenaire)
				.setUnlocalizedName("villageStone");
	...
	public static void render() {
		...
		renderItem.getItemModelMesher().register(Item.getItemFromBlock(villageStone), 0,
				new ModelResourceLocation(Reference.MOD_ID + ":villageStone", "inventory"));
	}
}
