...
public class StoredPosition extends Block {
	...
	public void randomDisplayTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		if (showParticles) {
			...
			worldIn.spawnParticle(EnumParticleTypes.SPELL_MOB, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D,
					d0, d1, d2, new int[0]);
		}
	}
	 ...
	 * tab, List list) { if (Block.getBlockFromItem(itemIn) == this) {
	 * StoredPosition.EnumType[] aenumtype = StoredPosition.EnumType.values(); int i
	 * = aenumtype.length;
	 * 
	 * for (int j = 0; j < i; ++j) { StoredPosition.EnumType enumtype =
	 * aenumtype[j]; list.add(new ItemStack(itemIn, 1, enumtype.getMetadata())); } }
	 * }
	 */
	...
	public static void preinitialize() {
		storedPosition = new StoredPosition().setCreativeTab(Millenaire.tabMillenaire)
				.setUnlocalizedName("storedPosition");
	...
	public static void render() {
		Minecraft.getMinecraft().getRenderItem().getItemModelMesher().getModelManager().getBlockModelShapes()
				.registerBuiltInBlocks(storedPosition);

		Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(Item.getItemFromBlock(storedPosition), 0,
				new ModelResourceLocation(Reference.MOD_ID + ":storedPosition", "inventory"));

		// ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(storedPosition),
		// 0, new ModelResourceLocation(Reference.MOD_ID + ":storedPosition",
		// "inventory"));
		// ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(storedPosition),
		// 1, new ModelResourceLocation(Reference.MOD_ID + ":storedPosition",
		// "inventory"));
		// ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(storedPosition),
		// 2, new ModelResourceLocation(Reference.MOD_ID + ":storedPosition",
		// "inventory"));
		// ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(storedPosition),
		// 3, new ModelResourceLocation(Reference.MOD_ID + ":storedPosition",
		// "inventory"));
		// ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(storedPosition),
		// 4, new ModelResourceLocation(Reference.MOD_ID + ":storedPosition",
		// "inventory"));
		// ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(storedPosition),
		// 5, new ModelResourceLocation(Reference.MOD_ID + ":storedPosition",
		// "inventory"));
	}
	...
	public static enum EnumType implements IStringSerializable {
		TRADEPOS(0, "tradePos"), SOURCEPOS(1, "sourcePos"), PATHPOS(2, "pathPos"), HIDEPOS(3, "hidePos"), DEFENDPOS(4,
				"defendPos"), SLEEPPOS(5, "sleepPos");
