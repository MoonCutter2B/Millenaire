...
public class GuiPanelParchment extends GuiText {
	...
	public GuiPanelParchment(final EntityPlayer player, final Building townHall, final List<List<Line>> fullTextLines, final int mapType, final boolean isParchment) {
		super();
	...
	public GuiPanelParchment(final EntityPlayer player, final List<List<String>> fullText, final Building townHall, final int mapType, final boolean isParchment) {
		super();
	...
	private void drawChunkMap(final int i, final int j) {
			...
			if (b.isTownhall && b.winfo != null && b.villageType != null) {
				for (int x = b.winfo.mapStartX; x < b.winfo.mapStartX + b.winfo.length; x += 16) {
					for (int z = b.winfo.mapStartZ; z < b.winfo.mapStartZ + b.winfo.width; z += 16) {
						if (x >= posXstart && x <= posXstart + chunkMapSizeInBlocks && z >= posZstart && z <= posZstart + chunkMapSizeInBlocks) {
							int colour;
		...
		for (final ChunkCoordIntPair cc : ForgeChunkManager.getPersistentChunksFor(world).keys()) {
			if (cc.chunkXPos * 16 >= posXstart && cc.chunkXPos * 16 <= posXstart + chunkMapSizeInBlocks && cc.chunkZPos * 16 >= posZstart && cc.chunkZPos * 16 <= posZstart + chunkMapSizeInBlocks) {
	...
	private void drawVillageMap(final int i, final int j) {
		...
		for (final MillVillager villager : townHall.villagers) {
			...
			if (mapPosX > 0 && mapPosZ > 0 && mapPosX < minfo.length && mapPosZ < minfo.width) {
				...
				if (villager.isChild()) {
					drawGradientRect(startX + mapPosX - 1, startY + mapPosZ - 1, startX + mapPosX + 1, startY + mapPosZ + 1, 0xA0ffff00, 0xA0ffff00);
				} else if (villager.getRecord() != null && villager.getRecord().raidingVillage) {
					drawGradientRect(startX + mapPosX - 1, startY + mapPosZ - 1, startX + mapPosX + 1, startY + mapPosZ + 1, 0xA0000000, 0xA0000000);
				} else if (villager.gender == MillVillager.MALE) {
					drawGradientRect(startX + mapPosX - 1, startY + mapPosZ - 1, startX + mapPosX + 1, startY + mapPosZ + 1, 0xA000ffff, 0xA000ffff);
				} else {
					drawGradientRect(startX + mapPosX - 1, startY + mapPosZ - 1, startX + mapPosX + 1, startY + mapPosZ + 1, 0xA0ff0000, 0xA0ff0000);
				}
		...
		if (villagerHover != null) {
			...
			if (unreachable) {
				stringlength = fontRendererObj.getStringWidth(locHover.getNativeName() + " - " + MLN.string("panels.unreachablebuilding"));
				nativeString = locHover.getNativeName() + " - " + MLN.string("panels.unreachablebuilding");
			} else {
				stringlength = fontRendererObj.getStringWidth(locHover.getNativeName());
				nativeString = locHover.getNativeName();
			}

			int nblines = 1;

			final boolean gameString = locHover.getGameName() != null && locHover.getGameName().length() > 0;

			if (gameString) {
				stringlength = Math.max(stringlength, fontRendererObj.getStringWidth(locHover.getGameName()));
				nblines++;
			}

			final List<String> effects = locHover.getBuildingEffects(townHall.worldObj);
			...
			if (gameString) {
				fontRendererObj.drawString(locHover.getGameName(), i - xStart, j - yStart + 11, 0x909090);
				pos++;
			}
